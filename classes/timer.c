...
LUA_OBJECT_FUNCS(timer_class, timer_t, timer)
 ...
 */
static void
luaH_timer_destroy(lua_State *L, timer_t *timer) {
...
luaH_timer_new(lua_State *L)
{
    ...
    timer_t *timer = luaH_checkudata(L, -1, &timer_class);
    timer->ref = NULL;
    timer->timer_id = TIMER_STOPPED;
    return 1;
}
...
static int
luaH_timer_get_started(lua_State *L, timer_t *timer)
{
